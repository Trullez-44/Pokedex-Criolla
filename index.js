...
document.addEventListener("click", async (e) => {
  ...
  if (e.target.matches(`.prueba`)) {
    ...
    var call_datos = await (await fetch(url + my_button.textContent/*EL ENDPOINT ES EL NOMBRE O ID DEL POKEMON */)).json();
    let pokemon_image = call_datos.sprites["front_default"];
    ...
    Swal.fire({
      title: `
      ${call_datos.species["name"].toUpperCase()}`,
  ...
  <form id="div_">
      ${call_datos.stats.map((iterador) => {
        return `<div data-name="${iterador.stat.name}" class="stats"><p class="inputs" data-name="${iterador.stat.name}">
          ...
          .join("")}
          <input type="submit" value="SEND INFO" form="div_" class="sending">
          </form>
      `,
    ...
    container.addEventListener("click", async (e) => {
      if (e.target.matches(`.sending`)) {
        // console.log('click');
        let res = await(await fetch(url + my_button.textContent/*EL ENDPOINT ES EL NOMBRE O ID DEL POKEMON */)).json();
        console.log(res);
      }
    })
  }
